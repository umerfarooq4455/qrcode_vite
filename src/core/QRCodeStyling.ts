...
export default class QRCodeStyling {
  ...
  constructor(options?: Partial<Options>) {
    this._options = options
      ? sanitizeOptions(mergeDeep(defaultOptions, options) as RequiredOptions)
      : defaultOptions;
    this.update();
  }

  static _clearContainer(container?: HTMLElement | null): void {
    if (container) {
      container.innerHTML = "";
    }
  }
  ...
  update(options?: Partial<Options>): void {
    ...
    this._options = options
      ? sanitizeOptions(mergeDeep(this._options, options) as RequiredOptions)
      : this._options;
  ...
  append(container?: HTMLElement | null): void {
    if (!container) {
      return;
    }
    ...
    if (this._options.type === drawTypes.canvas && this._canvas) {
      container.appendChild(this._canvas.getCanvas());
    } else if (this._options.type === drawTypes.svg && this._svg) {
  ...
  async getRawData(extension: Extension = "png"): Promise<Blob | null> {
    ...
    if (extension.toLowerCase() === "svg") {
      ...
      const source = serializer.serializeToString(
        (element as unknown as QRSVG).getElement()
      );
      ...
      return new Promise((resolve) =>
        (element as unknown as QRCanvas)
          .getCanvas()
          .toBlob(resolve, `image/${extension}`, 1)
      );
    }
  }
  ...
  async download(
    ...
    if (typeof downloadOptions === "string") {
      ...
      console.warn(
        "Extension is deprecated as an argument for 'download' method, please pass object { name: '...', extension: '...' } as an argument"
      );
    } else if (
      typeof downloadOptions === "object" &&
    ...
    if (extension.toLowerCase() === "svg") {
      ...
      const url = (element as unknown as QRCanvas)
        .getCanvas()
