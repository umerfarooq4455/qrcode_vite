...
const MyqrHeader: React.FC = () => {
  ...
  const initialOptions: string[] = ["5", "10", "25", "50"];

  const [isDropdownOpen, setIsDropdownOpen] = useState<boolean>(false);
  const [isCheckboxChecked, setIsCheckboxChecked] = useState<boolean>(false);
  const [isDropdownOpesort, setIsDropdownOpesort] = useState<boolean>(false);
  ...
  const [selectedType, setSelectedType] = useState<string>("5");
  ...
  const handleTypecheck = (value: string) => {
    setSelectedType(value);
  };

  const handleCheckboxChangesort = (value: string) => {
  ...
  const isGridView: boolean = viewMode === "grid";
  ...
  const handleTypeCheck = (id: number) => {
  ...
  const handleQrcodeCheck = (id: number) => {
