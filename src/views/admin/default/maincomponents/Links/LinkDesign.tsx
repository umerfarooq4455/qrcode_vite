...
const LinkDesign: React.FC = () => {
  const [activeIndex, setActiveIndex] = useState<number | null>(1);
  const [backgroudcolor, setBackgroudcolor] = useState<string[]>([
    "#292929",
    "#EDE728",
    "#28ED28",
    "#6D21B1",
  ...
  // Background color related states
  const [bgcolor, setBgcolor] = useState<string>("#292929");
  ...
  const [linkTcolor, setLinkTcolor] = useState<string>("#F18430");
  const [showLtcolorPicker, setShowLtcolorPicker] = useState<boolean>(false);
  const [colorLinktextIndex, setColorLinktextIndex] = useState<number | null>(null);

  // Link background color related state
  const [linkBgcolor, setLinkBgcolor] = useState<string>("#FFF8D4");
  const [showLinkBgcolorPicker, setShowLinkBgcolorPicker] = useState<boolean>(false);
  ...
  const handleBackgroundColorPicker = (color: any) => {
    const { hex } = color;
  ...
  const handleBackgroundInputChange = (color: string) => {
    const updatedBgcolorColors = [...backgroudcolor];
    updatedBgcolorColors[colorBgIndex as number] = color;
    setBackgroudcolor(updatedBgcolorColors);
    setBgcolor(updatedBgcolorColors[colorBgIndex as number]);
  };
  // Background color picker handlers

  // Link text background color handlers
  const handleLinkBgColorPicker = (color: any) => {
    const { hex } = color;
  ...
  const handleLinkBgInputChange = (color: string) => {
    const updatedLinktcolorColors = [...linkbgcolor];
    updatedLinktcolorColors[colorLinkbgIndex as number] = color;
    setLinkbgcolor(updatedLinktcolorColors);
    setLinkBgcolor(updatedLinktcolorColors[colorLinkbgIndex as number]);
  };
  // Link text background color handlers

  // Link text color picker handlers
  const handleLinkTextColorPicker = (color: any) => {
    const { hex } = color;
  ...
  const handleSecondaryInputChange = (color: string) => {
    const updatedSecondaryColors = [...linktextcolor];
    updatedSecondaryColors[colorLinktextIndex as number] = color;
    setLinktextcolor(updatedSecondaryColors);
    setLinkTcolor(updatedSecondaryColors[colorLinktextIndex as number]);
  };
  // Link text color picker handlers
        ...
        {activeIndex && (
          <div className="mb-1">
            ...
            <div className="grid w-full grid-cols-2 gap-3 pb-2 md:grid-cols-4 lg:grid-cols-4 xl:max-w-[86%] xl:grid-cols-6">
              {backgroudcolor.map((color, index) => (
                ...
                >
                  ...
                  <div
                    className="h-[32px] w-[35px] rounded-[3px] border dark:border-none"
                    style={{ backgroundColor: linktextcolor[index] }}
                  ></div>
                </div>
              ))}
            </div>
            ...
            <div className="mb-5 mt-4 flex flex-col md:flex-row">
              ...
              <div className="md:mx-3 relative flex flex-col">
                <label className="mb-1 text-[12px] font-semibold text-[#1B254B] dark:text-[#fff]">
                  Link text background color
                </label>
                <input
