...
const Contact: React.FC = () => {
  ...
  const [telephoneInputs, setTelephoneInputs] = useState<TelephoneInput[]>([
  ...
  const [websiteInputs, setWebsiteInputs] = useState<WebsiteInput[]>([
    { name: "Website label", link: "https://example.com" },
  ]);

  const addWebsiteInput = () => {
    setWebsiteInputs([...websiteInputs, { name: "", link: "" }]);
  };
  ...
  const handleWebsiteInputChange = (index: number, field: keyof WebsiteInput, value: string) => {
    const newInputs = [...websiteInputs];
  ...
  const addEmailInput = () => {
    setEmailInputs([...emailInputs, { name: "", email: "" }]);
  };

  const removeTelephoneInput = (index: number) => {
  ...
  const handleTelephoneInputChange = (index: number, field: keyof TelephoneInput, value: string) => {
    const newInputs = [...telephoneInputs];
    newInputs[index][field] = value;
        ...
        {activeIndex === 1 && (
          <div className="mb-1">
            <div className="mt-5 flex flex-col">
              ...
              {websiteInputs.map((input, index) => (
                <div
                  key={index}
                  className="mb-4  flex flex-col md:w-3/4 md:flex-row md:space-x-4"
                >
                  <div className="flex w-full flex-col md:w-1/2">
                    <label className="mb-2 block text-[12px] font-semibold text-gray-900 dark:text-white">
                      Website Label
                    ...
                    <input
                      ...
                      value={input.name}
                      onChange={(e) =>
                        handleWebsiteInputChange(index, "name", e.target.value)
                      }
                    />
                  </div>
                  <div className="mt-2 flex w-full flex-col md:mt-0 md:w-1/2">
                    ...
                    <input
                      ...
                      value={input.link}
                      onChange={(e) =>
                        handleWebsiteInputChange(index, "link", e.target.value)
                      }
                    />
                  </div>
                  <button
