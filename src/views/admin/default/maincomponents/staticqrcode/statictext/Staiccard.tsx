...
const Staiccard = ({ onChange }: { onChange: (event: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => void }) => {
  const {
    isInputValid,
    setIsInputValid,
    websiteUrl,
  ...
  const [parameters] = useState<{ id: number; name: string; value: string; }[]>([
  ...
  const [newCustomParam, setNewCustomParam] = useState<{ name: string; value: string; }>({
    name: "",
    value: "",
  });
      ...
      <div className="rounded-[10px] border-[1px] border-gray-300 bg-white px-5 font-dm text-[16px] font-semibold shadow-sm shadow-[#003EFF1A] dark:border-[#191A1F] dark:bg-[#13151E] dark:text-[#fff]">
        ...
        {activeIndex === 1 && (
          <div className="">
            ...
            <div className="mb-4 flex flex-col text-[#1B254B] dark:text-[#fff]">
              <span className="text-[14px] font-bold">Text</span>
              <span className="text-[12px] font-medium">
                Type any message containing numbers and symbols up to 300
                characters.
              </span>
            </div>
            <div className="mb-4 ">
