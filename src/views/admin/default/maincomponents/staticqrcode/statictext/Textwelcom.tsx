...
const Textwelcom = () => {
  ...
  const [selectedFilee, setSelectedFilee] = useState<string>(defultpdfimg);
  ...
  return (
    <div className="space-y-2">
      <div className="rounded-10px border-1 border-gray-300 bg-white px-5 font-dm text-16px font-semibold shadow-sm shadow-003EFF1A dark:border-191A1F dark:bg-13151E dark:text-white">
        <div
          className="flex h-45px cursor-pointer items-center justify-between"
          onClick={() => toggleAccordion(1)}
        >
          <span className="text-16px font-semibold text-1B254B dark:text-white">
            Welcome Screen
          </span>
          <svg
        ...
        {activeIndex === 1 && (
          <div className="mb-1">
              ...
              {isImageSelected ? (
                <>
                  <button
                    type="button"
                    onClick={deleteImage}
                    className="cursor-pointer rounded-3xl border border-5D5FEF p-3 px-7 text-sm font-semibold text-5D5FEF dark:border-white dark:text-white"
                  >
                    Delete
                  </button>
                  <button
            ...
            <div className="flex flex-col">
              <div className="mt-3 text-12px font-semibold text-gray-900 dark:text-white">
                Time (seconds)
              </div>
              <div>
                <input
