...
const Customframe = () => {
  ...
  useEffect(() => {
    if (qrcode && svgRef.current) {
      ...
      qrcode.append(foreignObject);
    }
  }, [qrcode, frameid]);

  useMemo(() => {
    if (textdata && textColor && svgRef.current) {
  ...
  const renderGradient = () => {
    if (isToggleButtonActive) {
      if (gradientType === "linear") {
        return (
          <linearGradient
            ...
            y2="0%"
          >
            <stop offset="0%" style={{ stopColor: calculateRotatedColor1() }} />
            <stop
              offset="100%"
              style={{ stopColor: calculateRotatedColor2() }}
            />
          </linearGradient>
        );
      ...
      if (gradientType === "linear") {
        return (
          <linearGradient id="linear-gradient" x1="0%" y1="0%" x2="0%" y2="0%">
            <stop offset="0%" style={{ stopColor: calculateRotatedColor1() }} />
          </linearGradient>
        );
      } else if (gradientType === "radial") {
        return (
          <radialGradient id="radial-gradient" cx="0%" cy="0%" r="0%">
            <stop offset="0%" style={{ stopColor: calculateRotatedColor1() }} />
          </radialGradient>
        );
      }
  ...
  const calculateRotatedColor1 = () => {
    const rotatedColor = Color(color1).alpha(0.5).lighten(0.5);
    return rotatedColor.hex();
  };

  const calculateRotatedColor2 = () => {
    const rotatedColor = Color(color2).rotate(range);
    return rotatedColor.hex();
  };

  const renderGradientbg = () => {
    if (isToggleButtonActive1) {
      if (gradientType === "linear") {
        return (
          <linearGradient
            id="linear-gradient"
            ...
            y2="0%"
          >
            <stop offset="0%" style={{ stopColor: calculateRotatedColor3() }} />
            <stop
              offset="100%"
              style={{ stopColor: calculateRotatedColor4() }}
            />
          </linearGradient>
        );
      } else if (gradientType === "radial") {
  ...
  const calculateRotatedColor3 = () => {
    const rotatedColor = Color(bgcolor1).alpha(0.5).lighten(0.5);
    return rotatedColor.hex();
  };

  const calculateRotatedColor4 = () => {
    const rotatedColor = Color(bgcolor2).rotate(range1);
    return rotatedColor.hex();
  };

  return (
    <>
      {frameid == 0 && <div id="canvas"></div>}
      {frameid == 1 && (
        <div
        ...
        >
          ...
          >
            ...
            <rect
              ...
              rx="2.5"
              stroke={`url(#${
                gradientType ? `${gradientType}-gradient` : "linear-gradient"
              })`}
            />
            <rect
              x="1"
